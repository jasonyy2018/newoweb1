...
function evaluateContent(id, pagePath, type, locale, title, corpus, hasConclusionFirstDirect, hasSchemaMarkup = true) {
...
async function main() {
    console.log('🚀 Running WSAI Enterprise GEO Compliance & Citation Readiness Audit...\n');

    const scores = [];
    const locales = ['zh', 'en', 'ja'];

    // Read geoData.ts for solutions
    let geoSolutions = {};
    const geoDataPath = path.join(projectRoot, 'src', 'lib', 'geo', 'geoData.ts');
    if (fs.existsSync(geoDataPath)) {
        const geoDataContent = fs.readFileSync(geoDataPath, 'utf-8');
        // Extract GEO_SOLUTIONS data or parse it
        try {
            const match = geoDataContent.match(/export const GEO_SOLUTIONS:[^{]+=\s*({[\s\S]*?});\n\nexport/);
            if (match) {
                // simple evaluate in function context
                geoSolutions = new Function(`return ${match[1]}`)();
            }
        } catch (e) {}
    }
    ...
    for (const locale of locales) {
        ...
        if (fs.existsSync(msgPath)) {
            try {
                ...
                for (const [slug, item] of Object.entries(solutionItems)) {
                    const geoEntry = geoSolutions[slug]?.[locale] || geoSolutions[slug]?.['en'] || null;
    ...
    for (const locale of locales) {
        ...
        if (fs.existsSync(msgPath)) {
            try {
                ...
                for (const [slug, item] of Object.entries(caseItems)) {
                    const resultsText = (item.results || [])
                        .map((r) => `${r.label}: ${r.value} (${r.desc})`)
                        .join(' ');
    ...
    console.log(`✅ GEO 审计完成！`);
    console.log(`- 扫描页面: ${totalPages}`);
    console.log(`- 平均评分: ${avg.toFixed(1)} / 100`);
    console.log(`- A+ 级页面: ${aPlusCount}`);
    console.log(`- A 级页面: ${aCount}`);
    console.log(`- B 级页面: ${bCount}`);
    console.log(`- 待优化页面: ${cCount}`);
    console.log(`- 完整报告已保存至: ${path.relative(projectRoot, reportPath)}\n`);
}
