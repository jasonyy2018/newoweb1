...
for entry in po:
    # 修复msgid是日文但应该是中文的条目
    if entry.msgid == "チーム紹介" and not entry.msgstr:
        entry.msgid = "团队介绍"
        entry.msgstr = "チーム紹介"
        fixed_count += 1
    elif entry.msgid == "アプリケーションコードエラー" and not entry.msgstr:
        entry.msgid = "应用程序代码错误"
        entry.msgstr = "アプリケーションコードエラー"
        fixed_count += 1
    elif entry.msgid == "データベース接続問題" and not entry.msgstr:
        entry.msgid = "数据库连接问题"
        entry.msgstr = "データベース接続問題"
        fixed_count += 1
    elif entry.msgid == "サードパーティサービス不可用" and not entry.msgstr:
        entry.msgid = "第三方服务不可用"
        entry.msgstr = "サードパーティサービス利用不可"
        fixed_count += 1
...
if len(untranslated) == 0:
    ...
    print("\n⚠️ 还有一些条目需要翻译:")
    for i, item in enumerate(untranslated[:5], 1):
        print(f"  {i}. {item}")
    if len(untranslated) > 5:
        print(f"  ... 还有 {len(untranslated) - 5} 个条目")