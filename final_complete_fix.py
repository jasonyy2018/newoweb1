...
# 修复那些msgid是日文但应该是中文的条目
fixed_count = 0
# 修复反向的条目（msgid是日文，应该恢复为中文）
reverse_fixes = {
    "チーム紹介": "团队介绍",
    "アプリケーションコードエラー": "应用程序代码错误",
    "データベース接続問題": "数据库连接问题",
    "サードパーティサービス不可用": "第三方服务不可用"
}

for entry in po:
    if entry.msgid in reverse_fixes and not entry.msgstr:
        # 保存当前的日文作为翻译
        japanese_text = entry.msgid
        # 恢复为正确的中文msgid
        entry.msgid = reverse_fixes[japanese_text]
        # 设置日文翻译
        entry.msgstr = japanese_text
        fixed_count += 1
        # 移除fuzzy标记
